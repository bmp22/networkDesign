...
#' @export
#'
gridSearchOld<-function(A,p,isomorphisms=FALSE,specialNodes=NULL, ignoreLastNode=FALSE, algorithm="sequential"){
  n<-dim(A)[1]
  b<-length(specialNodes)
  trialCE<-2
  numTrialsCE<-0
  successfulCE<-1
...
#  cat("\n")
  return(list("AOptVal"=AOptVal,"AOptDesign"=AOptDesign,"AOptNetVal"=AOptNetVal,"AOptNetDesign"=AOptDesign,"numEval"=numEval))
