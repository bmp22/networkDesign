...
#' @export
#'
gridSearchViral<-function(A,p,gammaPar=0.5,isomorphisms=FALSE,specialNodes=NULL, ignoreLastNode=FALSE, algorithm="sequential"){
  n<-dim(A)[1]
...
#      cat("\r",testDesign)
      ...
      X<-testWeight
      n<-dim(X)[1]
      F<-(solve((diag(n)-gammaPar*A))%*%cbind(rep(1,n),X))
      infMatrix<-(t(F)%*%F)
